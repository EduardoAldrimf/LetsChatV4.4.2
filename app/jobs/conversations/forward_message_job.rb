class Conversations::ForwardMessageJob < ApplicationJob
  ...
  def create_contact_inbox(contact)
    ::ContactInboxBuilder.new(contact: contact, inbox: @message.inbox, hmac_verified: true).perform
  end
  ...
  def forward_conversation(contact)
    @account.conversations.find_by!(contact_id: contact.id, inbox_id: @message.inbox_id)
  rescue StandardError
    ::Conversation.create!(conversation_params(contact))
  end
