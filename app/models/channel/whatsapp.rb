...
class Channel::Whatsapp < ApplicationRecord
  ...
  PROVIDERS = %w[default whatsapp_cloud baileys evolution].freeze
  ...
  after_destroy_commit :destroy_evolution_instance,
                       if: -> { provider == 'evolution' }
  ...
  def subscribe
    ...
    HTTParty.post(
      "https://graph.facebook.com/v22.0/#{provider_config['waba_id']}/subscribed_apps",
      headers: {
  ...
  def setup_evolution_instance
    return if ENV['EVOLUTION_API_URL'].blank? || ENV['EVOLUTION_ADMIN_TOKEN'].blank?

    setup_channel_provider
  rescue StandardError => e
    Rails.logger.error "Evolution setup failed: #{e.message}"
  end
